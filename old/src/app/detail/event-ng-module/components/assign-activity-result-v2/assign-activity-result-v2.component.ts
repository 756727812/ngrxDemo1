...
@Component({
  selector: 'app-event-assign-activity-result-v2',
  templateUrl: './assign-activity-result-v2.component.html',
  styleUrls: ['./assign-activity-result-v2.component.less'],
})
export class EventAssignActResultV2Component implements OnInit {
  ...
  reAssign(batchId, type) {
    this._isSpinning = true;
    this.dataService.ng_batchCopy_reAdd(batchId).then(
      res => {
        this._isSpinning = false;
        if (!res) {
          return;

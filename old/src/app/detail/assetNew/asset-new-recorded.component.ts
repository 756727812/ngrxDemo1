...
export class assetNewRecordedController {
  searchForm: any;
  page: string;
  total_items: number;
  list: any[];
  ...
  $onInit() {
    ...
    params['createTimeFrom'] = this.datePicker.startDate
      ? this.datePicker.startDate.format()
      : undefined;
    params['createTimeTo'] = this.datePicker.endDate
      ? this.datePicker.endDate.format()
      : undefined;
...
export const assetNewRecorded: ng.IComponentOptions = {
  template: require('./asset-new-record-list.template.html'),
  controller: assetNewRecordedController,
};

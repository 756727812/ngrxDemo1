...
import { goodsThemeGoodsListBtnFavor } from '../goods/theme/goods-list/btn-favor.component';

// 1=商品 2=拼团 3=秒杀 4=优惠劵 5=满减
...
@Component({
  selector: 'fashion-hot-goods-ng5-favor',
  templateUrl: './fashion-hot-goods-ng5-favor.component.html',
  styles: [
    `
      :host ::ng-deep .label {
        padding: 0;
      ...
      :host ::ng-deep .btn-back-hot-list {
        background: white;
        padding: 11px 18px;
...
export class FashionHotGoodsNg5Favor implements OnInit {
  ...
  private getCheckOptionFlagObject() {
    return this.checkOptions.reduce((acc, value, index, arr) => {
      let number = '1';
      if (['kol_flag'].includes(value.value)) {
        number = '2';
      }
      acc[value.value] = this.getCheckOptionFlag(value.value, number);
      return acc;
    }, {});
  }
  ...
  favorIds: string[] = [];
  get curPageFavorIds() {
    return this.favorIds.filter((id, index) => this.isCurentIds(index));
  }
  assignAllGoods: boolean = false;

  isCurentIds(index): boolean {
    const page = this.page;
    const pageSize = this.pageSize;
    return index >= pageSize * (page - 1) && index <= pageSize * page - 1;
  }

  // select_flag 0：按热度  1：按时间
  getItemTopRank(select_flag = 0, add_to_ori = false) {
    /* this.dataService
      .data_api_materialFavorItemList({ is_v2: 1, page: 1, page_size: 99999 })
      .then(
        ({ data }) => {
          // data_api_materialFavorItemList接口的page_size参数有问题，传10会返回9条数据！故一次性获取所有数据，前端分页！
          this.total_items = data.list.length;
          this.max_page = Math.ceil(this.total_items / this.pageSize);
          this.favorIds = data.list.map(v => +v.item_id);
          this.getFashionData(select_flag, add_to_ori);
        },
        res => {
          this.loading = 0;
          this.message.create('error', '获取收藏夹列表失败！');
        },
      ); */
    this.dataService.data_api_getCollectionItem({}).then(
      ({ data }) => {
        this.total_items = data.length;
        this.max_page = Math.ceil(this.total_items / this.pageSize);
        this.favorIds = data.map(v => +v.content_id);
        this.getFashionData(select_flag, add_to_ori);
      },
      res => {
        this.loading = 0;
  ...
  getFashionData(select_flag = 0, add_to_ori = false) {
    if (!this.curPageFavorIds.length) {
      if (this.page > 1) {
    ...
    console.log('curPageFavorIds', this.curPageFavorIds);
    ...
    const params = {
      ...
      keyword: this.curPageFavorIds.join(','),
    };
    ...
    return this.dataService.data_api_materialSelectItem(params).then(
      ({ data }) => {
        ...
        if (Boolean(data)) {
          if (!add_to_ori) {
            // 如果返回结果数与查询id数不一致，过滤无效id，并重新查询！
            /** 
             * 2018.7.10 - 暂不过滤无效ID, 过滤会导致下一页出现重复商品
             * Bug: https://www.tapd.cn/20089011/bugtrace/bugs/view?bug_id=1120089011001006276
            if (this.curPageFavorIds.length !== +data.count_item) {
              const resIds = data.list_item.map(v => v.item_id);
              const curIds = JSON.parse(JSON.stringify(this.curPageFavorIds));
              this.favorIds = this.favorIds.filter(
                id => !curIds.includes(id) || resIds.includes(id),
              );
              this.total_items = this.favorIds.length;
              this.max_page = Math.ceil(this.total_items / this.pageSize);
              this.getFashionData(select_flag, add_to_ori);
            } else 
            */
  ...
  assignAllFavor() {
    if (!this.favorIds.length) {
      this.message.create('warning', '收藏夹为空！');
      return;
    }
    this.assignAllGoods = true;
    this.selectKOLs();
  }
  ...
  batchChildgoods() {
    ...
    const BatchCopyAddReq = {
      ...
      sourceIds: this.assignAllGoods ? this.favorIds : this.materialItemIds,
  ...
  saveSelectInfo() {
    window['fashion_checkedMap_favor'] = this.checkedMap;
    window['fashion_selectGoods_favor'] = this.selectGoods;
  }

  restoreSelectInfo() {
    if (window['fashion_checkedMap_favor']) {
      this.checkedMap = window['fashion_checkedMap_favor'];
    }
    if (window['fashion_selectGoods_favor']) {
      this.selectGoods = window['fashion_selectGoods_favor'];
    }
  }

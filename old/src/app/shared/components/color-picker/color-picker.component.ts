...
export class ColorPickerComponent
  ...
  pickHue(event: MouseEvent) {
    ...
    this.value = this.validateHSB({
      h: Math.floor(
        360 * (150 - Math.max(0, Math.min(150, event.pageY - top))) / 150,
      ),
      s: 100,
      b: 100,
    });
  ...
  pickColor(event: MouseEvent) {
    ...
    let saturation = Math.floor(
      100 * Math.max(0, Math.min(150, event.pageX - left)) / 150,
    );
    let brightness = Math.floor(
      100 * (150 - Math.max(0, Math.min(150, event.pageY - top))) / 150,
    );
    this.value = this.validateHSB({
      h: this.value.h,
      s: saturation,
  ...
  show() {
    this.panelViewChild.nativeElement.style.zIndex = String(
      ++ColorPickerComponent.zindex,
    );
  ...
  alignPanel() {
    if (this.appendTo)
      position.absolutePosition(
        this.panelViewChild.nativeElement,

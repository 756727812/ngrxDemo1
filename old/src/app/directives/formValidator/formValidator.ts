    ...
    if (this.needBindKeydown) {
      $element.bind("keydown keypress", function(event) {
        if (event.which === 13) {
          ...
          if ((<any>currentInput).type && (<any>currentInput).type !== "textarea") {
            var button = $element.find("button");
            if (button && button[0]) {
              button[0].focus();
            }
            (<any>currentInput).focus();
      ...
      var validatorFn = function() {
        ...
        this.defaultShowError = function(elem, errorMessages) {
          ...
          if (!$next || !$next.hasClass("w5c-error")) {
            $elem.after('<span class="w5c-error"><i class="si si-danger"></i>&nbsp;' + errorMessages[0] + '</span>');
          }
        };
    ...
    .directive("w5cFormSubmit", ['$parse', function($parse) {
      return {
        ...
        link: function(scope, element, attr, ctrl) {
          ...
          (<any>ctrl).needBindKeydown = true;
          (<any>ctrl).enterKeydownFn = submitFn;
        }
      };
    }])
          ...
          var doValidate = function() {
            ...
            $http.get(url).success(function(data) {
              ...
              if (!state) {
                ...
                if (formCtrl[(<any>elem[0]).name]) {
                  formCtrl[(<any>elem[0]).name].w5cError = true;
                }
                if (!formCtrl.$errors) {
    ...
    .directive('w5cDynamicElement', ["$timeout", function($timeout) {
      return {
        ...
        link: function(scope, elm, attrs, ctrls) {
          var name = (<any>elm[0]).name, formCtrl = ctrls[2];
          if (name) {

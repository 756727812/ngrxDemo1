...
datetime.$inject = ['datetime', '$log', '$document'];

export function datetime(datetime, $log, $document) {
  ...
  function getNode(node, direction?) {
    ...
    while (
      node &&
      (node.token.type == 'static' || node.token.type == 'regex')
    ) {
      node = node[direction];
    }
    return node;
  ...
  function getRange(element, nodes, node?) {
    let selection = getInputSelection(element),
      i,
      range;
    for (i = 0; i < nodes.length; i++) {
      if (
        (!range &&
          nodes[i].offset + nodes[i].viewValue.length >= selection.start) ||
        i == nodes.length - 1
      ) {
        range = {
          element,
  ...
  function linkFunc(scope, element, attrs, ngModel) {
    let parser = datetime(attrs.datetime),
    ...
    const validMax = function(value) {
      return (
        ngModel.$isEmpty(value) ||
        angular.isUndefined(attrs.max) ||
        value <= new Date(attrs.max)
      );
    };
